...
若所有問題都屬於 🟢 正常狀態 → 跳至 2.7.8 寫入知識庫（僅記錄分析結果，不執行修正）。

### 2.7.4 深入根因分析（每個發現都必做）

對 2.7.3 分類的每個 🔴 和 🟡 項目，逐一進行深入分析：

#### 分析框架
對每個問題回答：
1. **根因**：為什麼會發生？是程式碼 bug、設定錯誤、外部依賴還是設計缺陷？
2. **模式**：是偶發還是規律性出現？與特定時段/任務類型有關嗎？
3. **影響**：對系統穩定性、執行效率、用戶體驗的實際影響程度？
4. **關聯**：是否與其他問題有共同根因？
...
#### 2.7.5.1 搜尋參考案例
- 用 WebSearch 搜尋相關問題的解決方案（如 "PowerShell Start-Job timeout best practice"）
- 用知識庫 hybrid search 查詢歷史審查筆記，避免重複修改或回退已修正的問題
...
| 2 | 副作用評估 | 不影響其他 Agent（daily-digest / todoist），不改變下游解析格式 |
| 3 | 語法正確 | PowerShell / Markdown / JSON 語法無誤 |
| 4 | 編碼規範 | .ps1 維持 UTF-8 with BOM，JSON 維持 UTF-8 |

> **決策門檻**：4 項全部通過 → 進入 2.7.6 備份與執行。任一項不通過 → 修正方案後重新驗審（最多 2 次），仍不通過則記錄為「待手動處理」跳至 2.7.8。
...
- 備份檔名規則：`原檔名.bak`（如 `run-todoist-agent.ps1.bak`）
- 若備份已存在則覆蓋（代表同一審查週期的多次嘗試）
- 備份在驗證通過後清理（2.7.7.3）

### 2.7.7 執行修正 + 驗證

#### 2.7.7.1 建立子 Agent 執行修正
用 **Write 工具**建立 `task_prompt.md`（UTF-8），為需要修正的問題建立子 Agent：
...
## 驗證清單
- [ ] 修改的檔案語法正確（PowerShell: 無語法錯誤 / JSON: 可正常解析）
- [ ] 修改的邏輯符合方案描述
- [ ] 未修改方案範圍外的檔案
- [ ] git diff 確認變更範圍正確

## 品質自評
完成修正後，確認驗證清單全部通過。若未通過，嘗試修正一次。
...
#### 2.7.7.2 修改後驗證（子 Agent 完成後，由主 Agent 執行）

子 Agent 完成後，主 Agent 自行驗證：
1. **差異檢查**：`git diff` 確認修改範圍與方案一致，無意外變更
2. **功能驗證**：若修改的是 .ps1 腳本，用 `powershell -Command "Get-Content '檔案路徑' | Out-Null"` 檢查可讀性
3. **回歸確認**：讀取修改的檔案，確認關鍵邏輯未被破壞

若驗證失敗：
- 用備份還原：`cp "檔案路徑.bak" "檔案路徑"`
- 記錄失敗原因，跳至 2.7.8

#### 2.7.7.3 清理備份
驗證全部通過後，刪除備份檔案：
```bash
rm -f D:/Source/daily-digest-prompt/*.bak
```

### 2.7.8 將分析結果與修改成果寫入知識庫
**使用 Skill**：`knowledge-query`

將**完整的審查過程**（分析 + 方案 + 修改結果）寫入 RAG 知識庫：
...
對每個可處理項目，建立 prompt 並執行。每次最多 2 項，超出留待下次。
