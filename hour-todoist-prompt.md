...
依 `config/frequency-limits.yaml` 執行（共 15 種自動任務，40 次/日上限）。
...
每日僅 ~11 個 auto-task slots，但 15 個任務合計 40 次上限。為確保每個任務都有機會執行，使用 **純輪轉** 而非順序掃描。
...
### 2.8 全部達上限
繞一圈（15 個任務）都已達上限 → 跳到步驟 5（通知：今日自動任務已達上限）。
