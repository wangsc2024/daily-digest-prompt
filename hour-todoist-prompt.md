...
2. 依 SKILL.md 指示呼叫 Todoist API v1（`/api/v1/`）查詢**當日 + 過期**待辦（含 overdue，避免昨日未執行任務遺漏）
...
> 使用 python 比對：`datetime.fromisoformat(due_datetime.replace("Z", "+00:00")) <= datetime.now(timezone.utc)`
...
**過濾後輸出摘要**：`📋 API 回傳 N 筆 → 截止日期過濾後 M 筆（未到執行時間 K 筆）→ 已關閉 ID 過濾後 J 筆`
...
依 `config/frequency-limits.yaml` 執行（共 18 種自動任務，45 次/日上限）。
