...
schedules:
  ...
  todoist-team:
    ...
    timeout: 4000
    retry: 0
    description: "Todoist 團隊模式（每 30 分鐘，整點及半點）"
  ...
  autonomous-harness:
    ...
    timeout: 60
    retry: 0
    description: "Agent Harness 自治週期（每 5 分鐘，build_plan + recovery_worker 消費佇列）"
...
| todoist-team | 每 30 分鐘 01:00-23:30（整點及半點） | run-todoist-agent-team.ps1 | 4000s (~67min) | Todoist 團隊模式（逾 30min 可能與下一班重疊，見 troubleshooting §1.1） |
...
- todoist-team 每 30 分鐘執行（整點與半點），單次 timeout 約 67 分鐘；若執行超過約 30 分鐘，可能與下一班 run 重疊，共用同一 `results/` 導致「Phase 2 結果缺失」。
- 手動執行時請避開整點/半點前後數分鐘；排查與預防見 `docs/troubleshooting.md` §1.1。

## Todoist 驅動任務（由每小時排程撿起）

| 任務名稱 | Todoist 到期時間 | 實際執行排程 | 標籤 | 說明 |
|---------|----------------|------------|------|------|
| 遊戲同步：game → game_web → GitHub | 每日 11:20 | todoist-team 11:00 或 11:30 撿起 | 網站優化, 遊戲開發 | 同步今日新增遊戲並部署 |

> **說明：** Todoist 任務 11:20 到期 → 11:00 或 11:30 的 todoist-team 排程查詢後執行。
