...
- 存在 → 讀 `summary.recent_3d_topics`（3 天冷卻判定）、`summary.by_type.tech_research`（飽和度判定）
- 僅在邊界判定時才讀完整 entries

**判定規則（必須遵守）：**
1. 若 `summary.recent_3d_topics` 中有與本次候選主題完全相同的 topic → **必須換主題**（3 天冷卻）
2. 若 `summary.by_type.tech_research` ≥ 3 → 優先探索冷門技術
3. 比對 `summary.recent_3d_topics`，避免跨類型重複（含 ai_deep_research, ai_github_research 等）
...
```
同時移除超過 7 天的舊 entry。
**同步更新 summary**：total+1、by_type.tech_research+1、recent_3d_topics 前插本次主題（保留最多 10 條）、last_updated 更新為今日日期。
