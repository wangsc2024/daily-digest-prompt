...
> 觸發條件：Todoist 無可處理項目且 ai_smart_city_count < 2
...
## 任務
研究 AI 應用於智慧城市的最新技術與案例，將報告寫入 RAG 知識庫。
...
## 執行後更新
1. 更新 `context/auto-tasks-today.json`：`ai_smart_city_count` + 1
2. 寫入 `state/todoist-history.json`：auto_tasks 加入 type=ai_smart_city 記錄
