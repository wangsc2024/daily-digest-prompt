...
用 Write 建立暫存 JSON 檔，curl 發送後刪除暫存檔。
...
```json
{
  "topic": "wangsc2025",
  "title": "✅ 自愈完成",
  "message": "修復 {M}/{N} 項（arch-decision 執行 {arch_success} 項）| {YYYY-MM-DD HH:mm}",
  "priority": 2,
  "tags": ["white_check_mark"]
}
```
（arch_success 為步驟 h 實際執行的 immediate_fix 數量；若 0 項則通知內可註明「arch-decision 0 項」）
...
```json
{
  "topic": "wangsc2025",
  "title": "⚠️ 自愈部分失敗",
  "message": "成功 {M}/{N} 項 | 失敗：{failed_items_summary} | 需關注",
  "priority": 3,
  "tags": ["warning"]
}
```
...
===DONE_CERT_BEGIN===
{
  ...
  "artifacts_produced": [],
  "quality_score": 4,
  "self_assessment": "自愈迴圈完成，修復 M/N 項（含 arch-decision arch_count 項）"
}
===DONE_CERT_END===
```
