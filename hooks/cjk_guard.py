...
# 格式：(日文碼位, 繁中碼位, 說明)
CJK_CORRECTIONS = [
    (0x5C02, 0x5C08, "專→專 (專案/專業)"),
    (0x4F53, 0x9AD4, "體→體 (整體/身體)"),
    (0x5B66, 0x5B78, "學→學 (學習/學術)"),
    (0x5B9F, 0x5BE6, "實→實 (實際/實作)"),
    (0x6C17, 0x6C23, "氣→氣 (氣候/氣息)"),
    (0x69D8, 0x6A23, "樣→樣 (樣式/樣本)"),
    (0x4F1A, 0x6703, "會→會 (會議/機會)"),
    (0x56FD, 0x570B, "國→國 (國家/國際)"),
    (0x7D4C, 0x7D93, "經→經 (經過/經驗)"),
    (0x8FBA, 0x908A, "邊→邊 (邊界/附近)"),
    (0x5BFE, 0x5C0D, "對→對 (對應/對話)"),
    (0x8A3C, 0x8B49, "證→證 (證明/證書)"),
    (0x8FBA, 0x908A, "邊→邊 (邊緣)"),
    (0x5099, 0x5099, None),  # 備 在日中相同，跳過
    (0x6307, 0x6307, None),  # 指 在日中相同，跳過
    (0x96A3, 0x96A3, None),  # 隣→鄰，但日文多用 隣
]

# 過濾掉碼位相同的（兩語共用字元）
CORRECTIONS = {
    jp: tc
    for jp, tc, _ in CJK_CORRECTIONS
    if jp != tc
}
