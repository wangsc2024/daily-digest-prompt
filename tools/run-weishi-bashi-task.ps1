# ============================================================
# 唯識八識任務：匯入知識庫 + 製作 Podcast + 上傳 R2
# 依 temp/cursor-cli-task-weishi-bashi.md 執行
# ============================================================
...
# --- Step 0: 確認報告存在 ---
$Report = Join-Path $AgentDir "docs\research\唯識八識深度洞察報告_20260314.md"
...
uv run --project $AgentDir python tools/create_import_json.py
...
try {
    ...
    Write-Host "  1. uv run python tools/create_import_json.py"
    ...
    Write-Host "  3. 取得筆記 id 後執行 article-to-podcast.ps1"
    exit 1
}
...
pwsh -ExecutionPolicy Bypass -File "tools\article-to-podcast.ps1" -NoteId $NoteId -Slug "weishi-bashi-20260314"
