...
- 若回傳 `{"status":"ok"}` → 用 Groq 批次翻譯標題（逐篇，篇間等 0.5 秒）：
  1. 用 Write 工具建立 `cache/groq-hn-translate.json`：`{"mode":"translate","content":"<英文標題>"}`
  2. 執行：`curl -s --max-time 20 -X POST http://localhost:3002/groq/chat -H "Content-Type: application/json; charset=utf-8" -d @cache/groq-hn-translate.json`
