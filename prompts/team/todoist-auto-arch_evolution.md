...
對 `improvement-backlog.json` 中每個 item，依以下決策樹分類：

```
effort=low AND 操作步驟明確（可用工具直接完成）→ immediate_fix
  └─ 例：YAML 注釋補強、新增依賴套件、建立 schema 檔案
...
**安全邊界**：`immediate_fix` 的 `fix_instructions` 必須是可用 Claude 工具（Read/Write/Edit/Bash）安全完成的操作。涉及以下項目的一律改為 `schedule_adr`：
- 修改核心腳本（run-*.ps1）
- 修改 hooks/*.py
- 刪除任何現有檔案
...
## 安全邊界（禁止自動修復）
以下問題僅記錄 schedule_adr，**禁止設為 immediate_fix**：
