...
- 兩階段搜尋都失敗（去重資訊不足，但 KB 服務仍在線，**第四步匯入不受影響**）→ 從教觀綱宗概論開始研究
- 有結果 → 仔細閱讀已有標題，避免重複
...
**確認主題後，立即用 Write 建立** `results/todoist-auto-jiaoguangzong.json`（status 填 `"in_progress"`）：
```json
{
...
## 最後步驟：更新結果 JSON（最終狀態）
用 Write 覆寫 `results/todoist-auto-jiaoguangzong.json`，將 status 改為最終值：
```json
{
  "agent": "todoist-auto-jiaoguangzong",
