...
### Step 0.5: 讀取系統審查連續記憶（任務延續性）

用 Read 讀取 `context/continuity/system-audit.json`（不存在則略過）。

從最近 7 筆 `records` 中提取（按日期排序，最近的優先）：
- **前次 `health_score`**：與今次比對，判斷健康分趨勢（上升/下降/持平）
- **前次 `top_issues`**：本次重點關注相同問題是否已解決
- **前次 `fixes_applied`**：確認前次修正項目是否已生效
- **`consecutive_decline_count`**：若連續多次下降（≥2），在報告中標記「⚠️ 系統健康持續下降，建議優先介入」
- **`trend`**（若有）：提供給報告摘要的趨勢描述

> **用途**：讓每次系統審查成為前次的延續，識別長期趨勢而非只看單點數據。
...
2. 計算 today（YYYY-MM-DD，使用本地時間 UTC+8：`pwsh -Command "Get-Date -Format 'yyyy-MM-dd'"`），在 `records[today]` 中寫入：
```json
{
  "run_id": "<當前時間戳或 run_id>",
  "completed_at": "<ISO 8601>",
  "health_score": <加權總分>,
  "grade": "<S|A|B|C|D>",
  "top_issues": ["<問題1（維度+分數）>", "<問題2>"],
  "fixes_applied": <自動修正數量>,
  "fixes_list": ["<修正項目1>", "<修正項目2>"],
  "trend_vs_prev": "<improved|declined|stable（與 Step 0.5 前次分數比較）>",
  "consecutive_decline_count": <若 trend=declined 則「Step 0.5 讀到的最近一筆 record 的 consecutive_decline_count」+1；若 trend 非 declined 則固定為 0>,
  "report_path": "docs/系統審查報告_<DATE>.md",
  "status": "completed"
}
```
3. 清理 `records` 中超過 30 天的舊記錄
4. 用 Write 工具完整覆寫 `context/continuity/system-audit.json`
