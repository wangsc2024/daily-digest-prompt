...
### 模式 D：楞嚴經自動研究鏈（Todoist 無待辦時觸發）
```
todoist（確認無任務）→ knowledge-query（查已有筆記）→ WebSearch → knowledge-query（匯入）→ ntfy-notify
```
1. 用 todoist 查詢後確認無可處理項目
2. 用 knowledge-query 查詢已有楞嚴經筆記，避免重複
3. 選取未研究的主題，用 WebSearch + WebFetch 蒐集資料
4. 用 knowledge-query 將研究成果匯入知識庫
5. 用 ntfy-notify 通報研究完成
