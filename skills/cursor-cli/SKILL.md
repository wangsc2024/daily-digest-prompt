...
**預設推薦 LLM**：`--model composer-2-fast`（[Composer 2 說明](https://cursor.com/zh-Hant/blog/composer-2)：與 Composer 2 同等智慧、較快版本；CLI 中 `agent models` 顯示為 `composer-2-fast`）。若需同系列較慢但較省輸出成本可改 `--model composer-2`。
...
2. **以 Cursor CLI 參照該任務檔執行**  
   - 例：`agent -p "依 temp/cursor-cli-task-xxx.md 執行任務" --workspace "D:\Source\daily-digest-prompt" --model composer-2-fast`  
   - 或先 Read 該檔，再以「請依下列任務說明執行：…」為 prompt 呼叫 `agent -p`（建議同樣加上 `--model composer-2-fast`）
...
```powershell
agent -p "重構 config 目錄下的 YAML 命名一致" --workspace "D:\Source\daily-digest-prompt" --model composer-2-fast
```
...
| 排程內呼叫 Agent | 可用 pwsh 執行 `agent -p "任務" --workspace ... --model composer-2-fast --trust`，注意 -p 具完整寫入權限，建議限定 workspace |
