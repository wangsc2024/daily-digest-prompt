...
每個 API 來源一個 JSON 檔案。**TTL 值以 `config/cache-policy.yaml` 為準**（以下為速查）：

| 來源 | 檔名 | TTL（有效期） | 降級時限 |
|------|------|--------------|---------|
| Todoist | `cache/todoist.json` | 45 分鐘 | 24 小時 |
| 屏東新聞 | `cache/pingtung-news.json` | 6 小時 | 48 小時 |
| HN AI 新聞 | `cache/hackernews.json` | 3 小時 | 24 小時 |
| 知識庫 | `cache/knowledge.json` | 1 小時 | 24 小時 |
| Gmail | `cache/gmail.json` | 6 小時 | 24 小時 |

> **注意**：實際 TTL 數值定義在 `config/cache-policy.yaml`，本表為快速參考。若兩者不一致，以 `cache-policy.yaml` 為準。
...
4. 若無快取可用（超過降級時限或從未快取）：
   - 回報錯誤，該區塊標注「服務暫時不可用」

## 降級決策表

所有 prompt（含 daily-digest、todoist、team mode）統一參照此表。**數值以 `config/cache-policy.yaml` 為準**：

| API 來源 | TTL | 降級時限 | 降級標記 |
|---------|-----|---------|---------|
| Todoist | 45 分鐘 | 24 小時 | `⚠️ 資料來自快取（HH:MM）` |
| 屏東新聞 | 6 小時 | 48 小時 | `⚠️ 資料來自快取（HH:MM）` |
| HN AI | 3 小時 | 24 小時 | `⚠️ 資料來自快取（HH:MM）` |
| 知識庫 | 1 小時 | 24 小時 | `⚠️ 資料來自快取（HH:MM）` |
| Gmail | 6 小時 | 24 小時 | `⚠️ 資料來自快取（HH:MM）` |
...
function get_data(source):
  ...
  try:
    ...
    // 3. API 失敗，嘗試降級
    if cache_file exists AND age_minutes <= degraded_ttl_minutes:
...
- **TTL 值來源**：永遠以 `config/cache-policy.yaml` 為準，本文件的表格為速查用途
