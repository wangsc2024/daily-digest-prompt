...
description: |
  ...
  Use when: 聊天室任務查詢、bot.js 佇列管理、Gun.js 任務認領與執行、wsc-bot 互動。
...
- **認證**：Bearer Token — `Authorization: Bearer $BOT_API_SECRET`
- **免認證端點**：`/api/health` 僅此一個

> **禁止使用 `echo $BOT_API_SECRET`** — Harness 會攔截。若 Token 未設定，直接在結果中記錄錯誤，不要嘗試讀取方式。
...
### 查詢待處理任務
```bash
curl -s -H "Authorization: Bearer $BOT_API_SECRET" \
...
| `BOT_API_SECRET` 未設定 | 用健康檢查端點（不需認證）取得基本狀態；任務認領記錄「認證未配置」 |
