...
若執行過程中遇到服務不可用或資料不足，依以下策略降級：

| 情境 | 降級策略 |
|------|---------|
| **WebSearch API 不可用** | 改用 KB 混合搜尋（POST /api/search/hybrid）查詢已存檔研究，tags 篩選學術相關筆記 |
| **KB API 不可用** | 僅輸出研究設計（研究問題、檢索策略、納入標準），報告標註「⚠️ 需人工執行檢索」 |
| **文獻不足（< 6 篇 A/B 級）** | 報告開頭加「⚠️ 證據強度：低」，附錄說明「僅 N 篇高品質文獻，結論為暫時性判斷」 |
| **WebFetch 超時或失敗** | 僅記錄 title + source，備註「全文未取得，依標題與摘要判斷」 |
