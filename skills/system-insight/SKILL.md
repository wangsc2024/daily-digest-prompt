...
- 報告每日最多產生 1 次，避免重複分析
- 健康門檻值應與 `config/benchmark.yaml` 保持同步（benchmark 為權威來源）
- behavior-patterns.json 為可選資料源，尚未有程式自動產出，對應 3 個指標設為 null 不影響整體報告
- Skill 總數變更時，需同步更新本文件步驟 2 表格與步驟 3 的 `total_skills` 欄位
