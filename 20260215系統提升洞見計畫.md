...
| 無自癒迴圈（只告警不修復） | HIGH | 發現問題 → ntfy 通知人 → 等人修 | 無自動修復能力 |
...
#### 提案 2.3：自癒迴圈引擎（核心創新）
**靈感來源**：Replit Agent 3 自癒迴圈 + Agentic SRE
...
### Phase 3：Skill 增強 + 自癒（5-8 天）
> 影響：新增 4 個 Skill + 3 個自動任務（exec_order #16-#18）、自癒迴圈上線
...
| 自癒迴圈 自動任務 #17 | templates/auto-tasks/self-heal.md + frequency-limits + team prompt + PS1 映射（含 system-insight.json 降級條款） | 4h |
...
**位置**：Phase 3 路線圖（第 902 行）「自癒迴圈 自動任務 #17 | templates/auto-tasks/self-heal.md + frequency-limits 更新」。
